...
DEFAULT_SYSTEM_PROMPT = (
    "You are a helpful assistant that identify allergy ingredient in the provided recipe. you will find allergen for each ingredient with the given context and reply with each ingredient allergen information "# in a Json format"
)
...
class MCPConfig(BaseModel):
    url: Optional[str] = Field(
        default="https://m2m-tool.fastmcp.app/",
        optional=True,
    )
    """The URL of the MCP server"""
    tools: Optional[List[str]] = Field(
        default=["allergen_tool"],
        optional=True,
    )
...
class AllergenEntry(BaseModel):
    ...
    alternative: str = Field(..., description="Alternative ingredient to replace this allergen")
class AllergyAnalysisResponse(BaseModel):
    """Structured output for food allergy analysis"""
    high_risk_allergen: List[AllergenEntry] = Field(
        description="List of allergen-risk mappings"
    )
    # recommendation: str = Field(
    #     description="Overall recommendation for handling this recipe"
    # )
# class AllergenResponse(BaseModel):
#     """Respond to the user with this"""

#     ingredient: str = Field(description="The ingredient name")
#     allergen: bool = Field(description="True if there is allergen, false if there is no allergen")    
#     detail: str = Field(description="allergen information detail ")    

# class AgentState(MessagesState):
#     # Final structured response from the agent
#     final_response: AllergenResponse
...
async def graph(config: RunnableConfig):
    ...
    return create_react_agent(
        ...
        response_format=AllergyAnalysisResponse
    )
